...
def load_dataset(dataSetName):
    # Load pre-saved train/test .npy files
    x_train = np.load(f"{dataSetName}_train_data.npy") / 255.0
    y_train = np.load(f"{dataSetName}_train_label.npy")
    x_test = np.load(f"{dataSetName}_test_data.npy") / 255.0
    y_test = np.load(f"{dataSetName}_test_label.npy")
